...
export default function Calendar() {
  ...
  const data = useRef([])
  const [firstData, setfirestData] = useState([])
  ...
  useEffect(
    () =>
      onSnapshot(query(collection(DB, 'calendar')), (snapshot) => {
        ...
        snapshot.docs.forEach((doc, i) => {
          if (doc.data()) {
            const st = doc.data().start.seconds * 1000
            const en = doc.data().end.seconds * 1000
            data.current[i] = {
              id: doc.data().id,
              ...
              // "Timestamp(seconds=1657119600, nanoseconds=0)"
  ...
  return (
    <Page title="SNP 일정관리">
      <Container maxWidth={themeStretch ? false : 'xl'}>
        <HeaderBreadcrumbs
          heading="SNP 일정관리"
          links={[{ name: 'Dashboard', href: PATH_DASHBOARD.root }, { name: '일정' }]}
        ...
        <DialogAnimate open={isOpenModal} onClose={handleCloseModal}>
          <DialogTitle>{selectedEvent ? 'Event 수정' : 'Event 추가'}</DialogTitle>
